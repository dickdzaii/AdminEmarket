...
export class SanphamComponent extends BaseComponent implements OnInit {
  ...
  loadPage(page) {
    ...
    this._api.get(this.api + '/all-by-shop/emarket-shop/' + page + '/' + this.size)
  ...
  search() {
    let keyWord = this.formsearch.get('tenSp').value ?? '%20';
    let maloai1 = this.formsearch.get('loai1').value ?? '%20';
    let maloai2 = this.formsearch.get('loai2').value ?? '%20';
    let lowToHighPrice = this.formsearch.get('ten').value ?? null;
    let newestFirst = this.formsearch.get('ten').value.toString() ?? null;
    let min = this.formsearch.get('giaMin').value ?? 0;
    let max = this.formsearch.get('giaMax').value ?? 0;
    let maloai = this.formsearch.get('loai').value ?? 0;

    this._api.get(this.api + '/search/result/' + keyWord + '/' + '%20' + '/' + maloai + '/' + maloai1 + '/' + maloai2 + '/' + min + '/' + max + '/' + this.index + '/' + this.size + '/' + lowToHighPrice + '/' + newestFirst)
      .takeUntil(this.unsubscribe).subscribe(res => {
        this.response = res;
        this.list_item = this.response.data;
        this.totalRecords = this.response.totalItems;
  ...
  onSubmit(value) {
    ...
    if (this.isCreate) {
      this.getEncodeFromImage(this.file_image).subscribe((data: any): void => {
        ...
        this._api.post(this.api + '/them', tmp).takeUntil(this.unsubscribe).subscribe(res => {
  ...
  public openUpdateModal(row) {
    ...
    setTimeout(() => {
      $('#SanphamModal').modal('toggle');
      this._api.get(this.api + '/detail/' + row.link).takeUntil(this.unsubscribe).subscribe((res: any) => {
        this.single_item = res;
